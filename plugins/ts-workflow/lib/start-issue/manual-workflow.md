...
6. **Verify**: `(cd "$WORKTREE_PATH" && npx convex codegen)` for Convex repos, then `$PM run build` (if the script exists) + `$PM run type-check` (or `npx tsc --noEmit`) + `$PM test` + `$PM run lint` (if the script exists) — each as `(cd "$WORKTREE_PATH" && ...)`
...
7. **Verify**: `(cd "$WORKTREE_PATH" && npx convex codegen)` for Convex repos, then `$PM run build` (if the script exists) + `$PM run type-check` (or `npx tsc --noEmit`) + `$PM test` + `$PM run lint` (if the script exists) — each as `(cd "$WORKTREE_PATH" && ...)`
