...
These workflow skills are shared by multiple agent surfaces. Describe the
...
3. Before that final response, preserve the current phase of any active loop
   state and pause it. Loop state is **optional** here — `loop-state.sh` ships
   with the language workflow plugins, not this one — so bind it only when the
   file is actually present in this plugin:

   ```bash
   if [ -f "${CLAUDE_PLUGIN_ROOT}/lib/loop-state.sh" ]; then
     source "${CLAUDE_PLUGIN_ROOT}/lib/loop-state.sh"
     pause_loop_for_driver "$ACTIVE_LOOP_STATE_FILE" "<missing intent>"
   fi
   ```

   If that file does not exist, skip the loop-state binding entirely — there is
   no loop to pause, and this is not an error. The stop rules below still apply
   unchanged.

   Use the active workflow's state-file variable in place of
   `ACTIVE_LOOP_STATE_FILE`. When the driver answers, call
   `resume_loop_after_driver` on the same file before continuing — again, only
   when `loop-state.sh` was sourced. If the workflow has other durable state but
   no active loop, persist an incomplete result and reason there.
