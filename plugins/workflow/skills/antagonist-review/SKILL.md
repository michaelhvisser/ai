...
argument-hint: "[PR-number|PR-URL] [--base <ref>] [--max-fix-rounds <n>] [--effort low|medium|high|xhigh] [--no-post] [focus ...]"
---
...
```bash
BASE_REF=""; MAX_FIX_ROUNDS=3; CODEX_EFFORT="xhigh"; PR_NUM=""; PR_REPO=""; FOCUS=""; POST_COMMENT=true
SKIP_NEXT=""
...
# Every gh call below is repo-qualified with -R "$PR_REPO"; without a URL the
# ambient repository is the target, resolved once here.
[ -n "$PR_REPO" ] || PR_REPO=$(gh repo view --json nameWithOwner --jq .nameWithOwner 2>/dev/null || true)
```
...
COMMENTS_BEFORE=$(gh pr view "$PR_NUM" -R "$PR_REPO" --json comments --jq '.comments | length' 2>/dev/null || echo skip)
...
COMMENTS_AFTER=$(gh pr view "$PR_NUM" -R "$PR_REPO" --json comments --jq '.comments | length' 2>/dev/null || echo skip)
...
```bash
gh pr comment "$PR_NUM" -R "$PR_REPO" --body "$(cat <<'EOF'
...
- The trailing footer (`*Generated by workflow antagonist-review*`) is intentional — it lets
  ...
  ```bash
  PREV=$(gh pr view "$PR_NUM" -R "$PR_REPO" --json comments \
